    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        for (const item of yield* authWellKnown.configs().pipe(Effect.orDie)) {
          const content = isRecord(item.content) && isRecord(item.content.config) ? item.content.config : item.content
          yield* merge(
            item.source,
            yield* loadConfig(JSON.stringify(content), { dir: item.dir, source: item.source }),
            "global",
          )
...
).pipe(Layer.provide(Substitution.defaultLayer), Layer.provide(AuthWellKnown.defaultLayer))
